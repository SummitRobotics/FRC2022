...
public class StatusDisplayWidget extends SubsystemBase {
	 ...
	 * 
	 * @param status the message to display. NOTE: status cannot contain any :'s
	 *               because of the way data is encoded
	 ...
	 * @param status the message to display. NOTE: status cannot contain any :'s
	 *               because of the way data is encoded
	 *               the takes no color and insted uses the color set with
	 *               {@link #SetDefaultColor(Color8Bit color)}
	 */
	...
	/**
	 * Internal wrapper for status messages, so that they can be sorted and the one
	...
	private class StatusMessage {
		 ...
		 * makes a new status
		 * 
		 * @param priority the integer priorty of the message. a higher number means a
		 *                 higher priority
