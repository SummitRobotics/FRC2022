...
public class CommandThreader extends CommandBase {
  ...
  /**
   * wraps a command inside a thred allowing it to be exicuted independently of the sceduler it is
   * important to not use the command in any way after it is passed
   *
   * @param command the command to be wrapped
   * @param period the period in ms for the command to be run at
   * @param priority the java thred priority for the command from 1-10 WARNING all setter methods
   *     the command calls must be syncronised or ONLY EVER used by the command or bad things will
   *     happen
   */
...
class Executor extends Thread {
  ...
  public void run() {
    ...
    while (!command.isFinished() && !isInterrupted()) {
      ...
      try {
        // sleeps the thred the remaining time so that the exicution period is
        // consistant
        ...
        if (sleepPeriod < 0) {
          ...
          System.out.println(
              String.format(
                  "Loop overran by %f ms", Math.abs(((double) (sleepPeriod)) / 1_000_000)));
