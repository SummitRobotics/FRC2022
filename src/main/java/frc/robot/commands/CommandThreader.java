...
public class CommandThreader extends CommandBase {
	 ...
	 *                 WARNING all setter methods the command calls must be
	 *                 syncronised or ONLY EVER used by the command or bad things
	 *                 will happen
	 */
	public CommandThreader(Command command, double period, int priority) {
		...
		// addRequirments is dumb and will not take in a set and get requirements
		...
		while (!command.isFinished() && !isInterrupted()) {
			...
			try {
				...
				if (sleepPeriod < 0) {
					...
					System.out.println(
							String.format("Loop overran by %f ms", Math.abs(((double) (sleepPeriod)) / 1_000_000)));
